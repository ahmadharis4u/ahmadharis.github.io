...
static char time_string[SIZE]; /* Variable to store time value as string */
...
void logger(int level, /*Logging level*/
	    char *log_msg /*logging message*/
	    )
{

	FILE *logfile;  /* File pointer for logfile */
	char buffer[BUFF_SIZE]; /* buffer to store time string*/
 	int attempts = ZERO;    /* loop count */

        /*Initialize the character array */
        memset(buffer, 0, BUFF_SIZE);


 	if (log_level >= level) /* Don't do debug messages with a
                                   lower priority than the program's
                                   debugging level */
        {
	    get_now_time();
  	    (void)snprintf(buffer, (size_t)SIZE,"\n%s|%d|%s--> ", 
					time_string, getpid(), program_name);
		    ...
		    if ((logfile = fopen(log_file, "a")) != NULL)
    		    {
     			...
     			(void)fflush(logfile);
                        ...
                        if(EOF == fclose(logfile))
                        {
                            ...
                            TRACE("\nTRACE: Exiting function logger with Failure\n");
                            return;
                        }
